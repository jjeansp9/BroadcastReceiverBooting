...
// 리시버는 4대 컴포넌트여서 반드시 AndroidManifest.xml에 등록해야함
public class MyBootingReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        // N버전(api 25)부터는 앱을 설치한 후 최소 한번은 사용자가 앱 런처(목록)화면에서 사용자가 직접 아이콘을
        // 눌러서 실행한 앱만이 부팅방송을 들을 수 있음음
       if(action.equals(Intent.ACTION_BOOT_COMPLETED)){
           Toast.makeText(context, "boot received", Toast.LENGTH_SHORT).show();

           // 보통 부팅이 완료되었을 때 내 앱(MainActivity)가 실행되도록 할 수 있음
...
//            context.startActivity(i);
           ...
           // 대신 Notification(알림)을 보여주고 알림창으로 통해 특정 액티비티가 실행되도록 권고함
            if(Build.VERSION.SDK_INT>=29){
                ...
                builder.setContentText("이제 Ex60앱의 MainActivity를 실행할 수 있습니다");
                builder.setSubText("앱 실행");

                builder.setAutoCancel(true); // 알림창을 클릭하면 자동 아이콘 없어짐

                // 알림창 눌렀을 때 실행할 MainActivity를 실행시켜주는 Intent객체 생성
                Intent i= new Intent(context, MainActivity.class);
                // 바로 실행하지 않기에 보류중인(Pending) Intent로 변환
                PendingIntent pendingIntent= PendingIntent.getActivity(context, 100, i, PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE); // 100은 크게 의미없이 작성한 것임
                ...
                Intent i= new Intent(context, MainActivity.class);
                i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK); // 기존 액티비티가 없는 상태에서 액티비티를 실행할 때 필수 설정
                context.startActivity(i);
            }
